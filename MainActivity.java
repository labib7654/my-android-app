...
public class MainActivity extends AppCompatActivity implements LocationListener {
    ...
    private void startLocationUpdates() {
        try {
            // Request location updates every 5 seconds or every 10 meters
    ...
    public void onLocationChanged(@NonNull Location location) {
        if (lastLocation != null) {
            float distance = lastLocation.distanceTo(location) / 1000; // Convert to kilometers
    ...
    protected void onPause() {
        super.onPause();
        // Stop location updates to save battery when the app is in the background
    ...
    protected void onResume() {
        super.onResume();
        // Restart location updates when the app comes to the foreground
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            startLocationUpdates();
        }
    }
    ...
    protected void onDestroy() {
        super.onDestroy();
        // Ensure location updates are stopped when the activity is destroyed
        if (locationManager != null) {
            locationManager.removeUpdates(this);
        }
    }
